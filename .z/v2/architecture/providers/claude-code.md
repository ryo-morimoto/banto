...
const claudeCodeProvider: ResumableProvider = {
  ...
  mode: "terminal",
  resume: true,
  modeSwitching: true,  // supports /plan and /build slash commands
  check: () => { ... },
...
| PreCompact | `{}` | event: { type: "compact", source: "hook", payload: { reason: "auto" } } |
...
CC supports `/plan` and `/build` slash commands for switching between read-only exploration and full development modes.

```typescript
async switchMode(mode: AgentMode): Promise<void> {
  const command = mode === "plan" ? "/plan\n" : "/build\n";
  this.writeTerminal(new TextEncoder().encode(command));
  // modeSwitched event emitted when CC acknowledges the mode change
  // (detected via PTY output pattern or hook event)
}
```

**Initial mode**: Passed via `--permission-mode plan` for Plan mode at startup, or default `--permission-mode default` for Build mode.
