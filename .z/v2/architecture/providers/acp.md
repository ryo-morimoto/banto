...
```typescript
{
  ...
  permissions: true,
  resume: false,  // agent 依存。初期値 false、ネゴシエーション後に更新
...
class AcpSession implements AgentSession {
  ...
  async start(prompt: string) {
    ...
    // resume 対応を更新
    if (this.agentCapabilities.resume) {
      this.provider.capabilities.resume = true;
    }
  ...
  private buildCommand(): string[] {
    // エージェント固有の ACP 起動コマンド
    // 設定ファイルで定義: { command: "opencode", args: ["--acp"] }
...
| permission/requested | permission_request | `{ requestId, tool, args, description }` |
...
| resume 対応 | エージェント依存 | なければ capabilities.resume = false |
